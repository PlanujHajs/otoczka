...
export function ConvexHullCalculator() {
  ...
  const drawCanvas = () => {
    ...
    const drawLine = (x1: number, y1: number, x2: number, y2: number, bold = false) => {
    ...
    for (let v = -maxRange; v <= maxRange; v += step) {
      ...
      if (Math.abs(pixel) <= canvas.width / 2) {
        // Pionowe linie
        drawLine(center.x + pixel, 0, center.x + pixel, canvas.height, isMajorLine);
        // Poziome linie
        drawLine(0, center.y - pixel, canvas.width, center.y - pixel, isMajorLine);
      }
    }
    ...
    for (let v = -maxRange; v <= maxRange; v += step) {
      ...
      if (isMajorLine && v !== 0 && Math.abs(pixel) <= canvas.width / 2) {
        ...
        if (!drawnLabels.has(labelKey) && Math.abs(pixel) > minLabelDistance / 2) {
    ...
    points.forEach(p => {
      ...
      ctx.beginPath();
      ctx.arc(center.x + p.x * scale, center.y - p.y * scale, 4, 0, 2 * Math.PI);
      ctx.fill();
    });
    ...
    if (hullPoints.length > 1) {
      ...
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.moveTo(center.x + hullPoints[0].x * scale, center.y - hullPoints[0].y * scale);
   ...
   * @param hull - Tablica punktów tworzących otoczkę wypukłą
   */
